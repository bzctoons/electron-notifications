...
class Notifier {
  ...
  notify (title, data) {
    ...
    const options = this.options = Object.assign({}, data, this._options);
    log.info(options);
	...
	const topWindow = remote.getCurrentWindow();
    log.info(`verticalSpace:${verticalSpace}, size.height:${size.height}`);
    let windowOptions = {
      width: 400,
      height: this.notificationHeight + verticalSpace,
      x: size.width - 400-32,
      y: size.height- this.notificationHeight + verticalSpace - 32, 
      frame: false,
      resizable: false,
      alwaysOnTop: true,
      skipTaskbar: true,
  ...
  showNext () {
    ...
    var notificationY = this.options.bottom;
